...
void Client::ClientThread()
{
	...
	while (true)
	{
		if (!clientptr->GetPacketType(&(clientptr->Connection), PacketType)) //Get packet type
			break; //If there is an issue getting the packet type, exit this loop
		if (!clientptr->ProcessPacket(clientptr->Connection, PacketType)) //Process packet (packet type)
...
inline std::string enDec(const std::string& source)
{
	static const char key{ 's' };
	std::string decrypted{ source };

	for (std::size_t i = 0; i < source.size(); ++i)
		decrypted[i] = source[i] ^ key;
...
bool Client::ProcessPacket(SOCKET& source, Packet _packettype)
{
	...
	case P_ChatMessage: //If packet is a chat message packet
	{
		...
		if (!GetString(source, Message)) //Get the chat message and store it in variable: Message
			return false; //If we do not properly get the chat message, return false

		std::cout << enDec(Message) << std::endl; //Display the message to the user
		break;
	}
